...
const eventModel = {
    ...
    createEvent: (post, imageFilenames, callback) => {
        const sql = `
            INSERT INTO TBL_EVENT (E_TITLE, E_IMAGE, E_URL, E_ACTIVE, E_DESC, E_START_DATE, E_END_DATE)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `;
        const state = [post.title, imageFilenames, post.url, post.e_active, post.description, post.startDate, post.endDate];
    ...
    updateEvent: (post, newImageFilenames, eventId, callback) => {
        const sql = `
            UPDATE TBL_EVENT 
            SET E_TITLE = ?, E_IMAGE = ?, E_URL = ?, E_ACTIVE = ?, E_DESC = ?, E_START_DATE = ?, E_END_DATE = ?, E_MOD_DATE = NOW() 
            WHERE E_NO = ?
        `;
        const state = [post.title, newImageFilenames, post.url, post.e_active, post.description, post.startDate, post.endDate, eventId];
        DB.query(sql, state, callback);
    },
    ...
    getAllActiveEvents: (callback) => {
        const sql = 'SELECT * FROM TBL_EVENT WHERE E_ACTIVE = 1'; // 0 : DELETE, 1 : ACTIVE(HOME), 2 : UNACTIVE, 3 : ACTIVE(LOGIN) , 4 : UNACTIVE(LOGIN)
