...
const chatService = {
    ...
    adminChatList: (req, res) => {
        ...
        chatModel.findAdminChatRooms(req.user.U_ID, (error, chattingRooms) => {
    ...
    // 관리자 채팅 기록 조회
    getAdminChatHistory: (roomId, senderId, senderNick, validExitTime) => {
        return new Promise((resolve, reject) => {
            
            console.log("********validExitTime********: ", validExitTime);
            chatModel.findChatMessage(roomId, senderId, senderNick, validExitTime, (error, results) => {
                if (error) {
